...
\subsection{Wp³yw na wydajno¶æ biblioteki}

  Nale¿y dokonaæ bardzo istotnego spostrze¿enia dotycz±cego przekazywania wyra¿enia do obliczenia przy pomocy szablonu wyra¿enia.
  Mianowicie u¿ycie tej metody nie wp³ywa znacz±co na wyd³u¿enie wykonania programu.
  Zdecydowana wiêkszo¶æ operacji zwi±zanych z tworzeniem szablonu wyra¿enia zachodzi w czasie kompilacji, gdy¿ proces zachodzi poprzez obliczenia na typach.
  Ponadto, przy dobrze dzia³aj±cym mechani¼mie rozwijania funkcji w miejscu wykonania, równie¿ obliczenie warto¶ci wyra¿enia odbêdzie siê w optymalny sposób,
  bez utraty wydajno¶ci w stosunku do standardowej postaci wyra¿enia.
...
\subsection{Procedura ewaluacji}
  ...
  Najbardziej standardowym rozwi±zaniem by³oby umieszczenie w ka¿dym obiekcie\\ \texttt{deferred\_expr} flagi wraz mutexem j± chroni±cym w celu zapobie¿enia podwójnej ewaluacji wyra¿enia.
  ...
  Przekazanie tej funkcji zmiennej typu \verb|once_flag| wraz z funkcj± do wykonania (w tym przypadku funkcj± \\ \texttt{deferred\_expr::evaluate}) sprawia, ¿e dla danej flagi funkcja wykona siê tylko raz.
...
\subsubsection{Pobranie warto¶ci}
  ...
  Obiekt typu \dval pozwala na pobranie warto¶ci poprzez wywo³anie metody\\ \texttt{get\_value} lub te¿ niejawnie, poniewa¿ poprzez operator konwersji do typu, który reprezentuje.
...
\subsubsection{Implementacja przeci±¿ania operatorów}
  ...
  \begin{lstlisting}
   ...
   { 
      return make_deferred_value_from_exp(lazyf(
	&deferred_value<T>::get_value, this) + rhs); 
   }
  \end{lstlisting}
