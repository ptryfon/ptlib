...
\subsection{Idiom C++ szablonu wyra¿enia}
  ...
  Zaprezentuje ideê szablonu wyra¿enie na przyk³adzie, w którym jedyna dozwolon± operacj± bêdzie dodawanie liczb ca³kowitych lub zmiennoprzecinkowych.
\begin{lstlisting}
...
\end{lstlisting}
  ...
  Wzorzec \verb|Term| (linie 6-11) umo¿liwia oznaczenie dowolnej warto¶ci jako li¶cia w drzewie reprezentujacym szablon wyra¿enia. \footnote{Jednak wzorce \texttt{Term} bêd± zaliczane do wez³ów po¶rednich, warto¶æ, któr± on trzyma jest li¶ciem.}
...
\subsubsection{Szablon wyra¿enia w dzia³aniu}
  ...
  Ponadto, nie da siê tego zmieniæ przy pomocy prze³adowywania operatorów, gdy¿ co najmniej jednym argumentem operatora musi byæ klasa lub typ wyliczeniowy.
...
\subsection{Praktyczna implementacja szablonu wyra¿enia -- Boost.Proto}
  ...
  \begin{itemize}
   ...
   \item Prze³adowania operatorów dla budowy AST z wyra¿enia
  ...
  Biblioteka Parallel korzysta z Boost.Proto do tworzenia szablonów wyra¿eñ w postaci AST, równie¿ przy pomocy mechanizmu prze³adowywania operatorów, dziêki czemu budowa AST jest uproszczona.
...
\subsection{Ograniczenia mechanizmu ewaluacji}
  ...
  W zwi±zku ze sposobem przekazywania wyra¿eñ do obliczenia, mo¿liwo¶ci projektowania mechanizmu ewaluacyji by³y do¶æ powa¿nie ograniczone.
...
\subsubsection{Pobranie warto¶ci}
  ...
  Ponadto u¿ywanie obiektów typu \dval w wyra¿eniach bez jawnego pobierania warto¶ci ma bardzo istotne znaczenie dla pe³nego wykorzystania funkcji prze³adowuj±cych operatory dla wzorca typu \dval.
  Ta kwestia zostanie wyja¶niona w nastêpnej sekcji.
  
\subsection{Prze³adowanie operatorów szablonu typu \dval}

\subsubsection{Motywacja dla prze³adowywania operatorów}
  ...
  \begin{lstlisting}[numbers=none, frame=none]
   ...
   Aby to uzyskaæ wystarczy prze³adowaæ operator dodawania, w taki sposób, 
...
\subsubsection{Implementacja prze³adowywania operatorów}

  Jêzyk C++ pozwala na prze³adowanie wszystkich operatorów, oprócz \verb|::|, \verb|.| oraz \verb|.*|.
  Nie dla wszystkich pozosta³ych operatorów prze³adowywanie ich dla obiektów \dval ma sens.
  ...
  Oto pe³na lista prze³adowanych operatorów:
  \begin{verbatim}
   +  -  *  /  %  ^  &  |  ~  !  =  <  >  <<   >>  +=  -=  *=  /=  %=  ^=  
   &=  |=  >>=  <<=  ==  !=  <=  >=  &&   ||  ++  --  ->*  ->  []  ()
  \end{verbatim}
